...
function getCurrentIdentity(configPath, { env = process.env, spawn = defaultSpawnSync } = {}) {
  ...
  // WA_PASSTHROUGH lets the `wrangler` PATH shim (if installed) run the real
  // binary instead of blocking this internal identity probe.
  const result = spawn('wrangler', ['whoami'], {
    encoding: 'utf8',
    env: { ...env, WA_PASSTHROUGH: '1' },
  });
